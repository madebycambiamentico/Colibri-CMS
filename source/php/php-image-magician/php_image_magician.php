...
## -------------------------------------------------------- 
	...
	$error = '';
		...
		$compressionLevel = round(($imageQuality/100) * 9);

		// *** Invert qualit setting as 0 is best, not 9
		//$invertScaleQuality = 9 - $scaleQuality;
		//madebycambiamentico:
		//png has NO quality indicator (it's loseless quality).
		//istead has step of compression. 0 = no compression, 9 = full compression
		//it could be said that more compression is better "quality"

		$this->checkInterlaceImage($this->isInterlace);
		if (imagetypes() & IMG_PNG) {
		   imagepng($this->imageResized, $savePath, $compressionLevel);
		} else { $error = 'png'; }
				break;
