...
foreach ($_POST['user_guest'] as $id => $user){
	switch ($user['action']){
...
if (!empty($idsToDel)){
	$pdores = $pdo->query("DELETE FROM utenti WHERE id IN (".implode(',',$idsToDel).") AND classe = 0") or
...
//request ids and check the class is 0
$idsToCheck = array_keys($users);
$pdores = $pdo->query("SELECT id FROM utenti WHERE id IN (".implode(',',$idsToCheck).") AND classe = 0", PDO::FETCH_NUM) or
